...
func (p *Postgres) GetPasswordHash(id string) (pwd string, err error) {
	row := p.connPool.QueryRow(`SELECT password FROM users WHERE id = $1`, id)
	err = row.Scan(&pwd)
	return pwd, errors.Wrapf(err, "failed to read %s user's password", id)
}
